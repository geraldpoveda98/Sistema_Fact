...
import {
    ...
    Save, FileText, User, MapPin, Phone, Mail, Box, ArrowLeft, Eye, XCircle, FileInput, Printer
} from 'lucide-react';
...
const Facturas = () => {
    ...
    const [proformasEmitidas, setProformasEmitidas] = useState([]);
    const [formatosImpresion, setFormatosImpresion] = useState([]);
    const [formatoSeleccionado, setFormatoSeleccionado] = useState('');
    const [modalReimpresion, setModalReimpresion] = useState(null); // {doc, tipo} para el mini-modal de formato

    // Búsqueda para listas
    ...
    const initialStateForm = {
        ...
        proformaIdOrigen: null // ID de Proforma si fue importada
    };
    ...
    const cargarDatosMaestros = async () => {
        try {
            ...
            // FILTRO ESTricto para FACTURAS
            const seriesFactura = resSeries.data.filter(s => s.tipo === 'Factura' && s.condicion);
            setSeries(seriesFactura);
            ...
            const formatosActivos = resFormatos.data.filter(f => f.estado && f.tipo_documento === 'Factura');
            ...
            await cargarFacturasYProformas();

            if (resCajas.data.length > 0) handleFormDataChange('caja', resCajas.data[0]._id);
            if (seriesFactura.length > 0) handleSerieChange(seriesFactura[0]);
    ...
    // ======= LOGICA DE IMPORTACION DE PROFORMA =======
    const importarProformaAlCarrito = (proformaObj) => {
        // 1. Setear Cliente
        if (proformaObj.cliente) {
            handleClienteChange(proformaObj.cliente._id || proformaObj.cliente);
        }

        // 2. Setear Impuesto
        if (proformaObj.impuesto) {
            const impDB = impuestos.find(i => i._id === (proformaObj.impuesto._id || proformaObj.impuesto));
            if (impDB) handleImpuestoChange(impDB);
        }

        // 3. Setear Tipo Pago y Días
        if (proformaObj.tipo_pago) handleFormDataChange('tipo_pago', proformaObj.tipo_pago);
        if (proformaObj.diasCredito) handleFormDataChange('diasCredito', proformaObj.diasCredito);

        // 4. Link visual
        handleFormDataChange('proformaIdOrigen', proformaObj._id);

        // 5. Mapear Detalles al Carrito (Recrear el objeto articulo desde el catálogo local para asegurar estuctura)
        const nuevoCarrito = proformaObj.detalles.map(d => {
            const artID = (typeof d.articulo === 'object') ? d.articulo._id : d.articulo;
            const artCompleto = articulos.find(a => a._id === artID) || { _id: artID, nombre: 'Artículo Desconocido/Borrado', precio_venta: d.precio_venta };
            return {
        ...
        showToast("Proforma importada exitosamente. Revise los datos y edítelos si es necesario.", "success");
    };

    // ======= CARRITO =======
    const toggleArticulosModal = () => {
        setIsArticulosModalOpen(!isArticulosModalOpen);
        if (!isArticulosModalOpen) setTimeout(() => searchInputRef.current?.focus(), 100);
    };
    ...
    const agregarAlCarrito = (articulo, evt = null) => {
        ...
        if (articulo.stock <= 0) {
            showToast(`El repuesto '${articulo.nombre}' no tiene existencias en inventario.`, "error");
            return;
        }
    ...
    const proformasFiltradasImport = proformasEmitidas.filter(p =>
        p.numero.includes(proformaSearchTerm) ||
        (p.cliente && p.cliente.nombre.toLowerCase().includes(proformaSearchTerm.toLowerCase()))
    );
    ...
    const handleGuardarImprimir = async () => {
        ...
        const payload = {
            ...
            impuesto_obj: formData.impuestoId,
        ...
        try {
            ...
            const factGuardada = JSON.parse(JSON.stringify(res.data.factura));
            factGuardada.cliente = clientes.find(c => c._id === formData.clienteId);
            factGuardada.caja = cajas.find(c => c._id === formData.caja);
            factGuardada.impuesto_obj = impuestoActual;
            factGuardada.detalles = factGuardada.detalles.map(d => ({
                ...d, articulo: articulos.find(a => a._id === d.articulo) || { nombre: 'Desconocido' }
            }));

            generarPDF(factGuardada);

            // Reset
            setCarrito([]);
            setFormData(initialStateForm);
            setClienteDetalle({ documento: '', direccion: '', telefono1: '', telefono2: '', email: '' });
            // Recargar el correlativo para que la próxima factura use el número correcto
            if (series.length > 0) {
            ...
            console.error(error);
            showToast(error.response?.data?.error || "Error al procesar la factura", "error");
        }
    };
    ...
    const handleAnularFactura = async (id) => {
        if (window.confirm("¿Seguro que desea anular esta FACTURA? Esto devolverá el stock de todos los artículos al inventario.")) {
            try {
                ...
                showToast(error.response?.data?.error || "Error anulando factura", "error");
            }
        }
    };

    const handleVerPDFExistente = (facturaObj, formatoId = null) => {
        const fId = formatoId || formatoSeleccionado;
        const factEnriquecida = JSON.parse(JSON.stringify(facturaObj));
        factEnriquecida.detalles = factEnriquecida.detalles.map(d => {
        ...
        generarPDF(factEnriquecida, fId);
    };

    // ======= MOTOR PDF - FORMATO CARTA =======
    const generarPDF = async (dataFactura, formatoIdOverride = null) => {
        try {
            console.log("Iniciando generación de PDF Carta (Factura)...", dataFactura);
            ...
            if (esTicket) {
                await generarTicketPOS(dataFactura, formatoObj);
                return;
            }
            ...
            // ---- Datos del documento ----
            const clienteObj = dataFactura.cliente || {};
            const impuestoObj = dataFactura.impuesto_obj || null;
            ...
            if (empLogoUrl) {
                try {
                    ...
                    console.warn("Fallo carga logo Factura, omitiendo:", e);
            ...
            doc.setLineWidth(1);
            doc.line(margen, y, pageW - margen, y);            // ===== TÍTULO DEL DOCUMENTO =====
            y += 15;
            const tipoDoc = dataFactura.tipo_venta === 'Crédito'
                ? `FACTURA CRÉDITO (${dataFactura.dias_credito || 0} DÍAS)`
                : 'FACTURA CONTADO';
            ...
            doc.setFontSize(11).setFont('helvetica', 'bold').setTextColor(150, 0, 0); // Rojo para factura numero
            doc.text(`N° ${dataFactura.serie || 'EXP'}-${dataFactura.numero || '000'}`, numBoxX + numBoxW / 2, numBoxY + 14, { align: 'center' });
            doc.setTextColor(0, 0, 0);
            ...
            const fechaReal = new Date(dataFactura.fecha || Date.now());
            doc.setFontSize(8).setFont('helvetica', 'normal');
            ...
            const tableRows = (dataFactura.detalles || []).map(item => {
            ...
            console.log("Generando tabla Factura...");
            autoTable(doc, {
                head: [tableColumn],
                body: tableRows,
                startY: y,
            ...
            const subtotalVal = dataFactura.total_ventas || 0;
            const ivaVal = dataFactura.impuesto_total || 0;
            const totalCORD = dataFactura.total_factura || 0;
            const totalUSD = tipoCambio > 0 ? (totalCORD / tipoCambio) : 0;

            const txtPie = formatoObj?.mensaje_pie || empresa?.mensaje_factura || 'GRACIAS POR SU COMPRA. Toda devolución o reclamo requiere presentación de factura original.';
            const adicionalesLines = doc.splitTextToSize(String(txtPie), adicionalBoxW - 12);
            ...
            doc.setFontSize(7.5).setFont('helvetica', 'normal').setTextColor(30, 60, 30);
            const maxLineas = Math.min(adicionalesLines.length, 5);
            adicionalesLines.slice(0, maxLineas).forEach((line, i) => {
            ...
            doc.setFont('helvetica', 'bold').setTextColor(0, 60, 0);
            ...
            console.log("Abriendo PDF Factura...");
            ...
            if (!newWindow || newWindow.closed || typeof newWindow.closed === 'undefined') {
                ...
                doc.save(`Factura_${dataFactura.numero || timestamp}.pdf`);
                setToastMessage({ text: "Ventana emergente bloqueada. El PDF se ha descargado automáticamente.", type: 'success' });
            }

        } catch (error) {
            console.error("ERROR GENERANDO PDF FACTURA:", error);
            setToastMessage({ text: "Error crítico al generar el PDF. Revisa la consola.", type: 'error' });
        }
    };

    const generarTicketPOS = async (dataFactura, formatoObj) => {
        try {
            // Cálculo de altura dinámica según items
            const numItems = dataFactura.detalles.length;
            const hasLogo = !!empresa?.logoUrl;
            const logoH = hasLogo ? 60 : 0;
            const baseHeight = 320 + logoH; // altura base ajustada
            const itemHeight = 25; // reducción ligera de espacio por item
            const totalHeight = baseHeight + (numItems * itemHeight) + 120; // margen extra para totales y cambio
            ...
            if (empresa?.logoUrl) {
                try {
                    ...
                    console.warn("Error cargando logo en ticket:", e);
                }
            }
            ...
            if (empresa?.mensaje_factura) {
                const subHeaderLines = doc.splitTextToSize(empresa.mensaje_factura, width);
            ...
            let tipoText = dataFactura.tipo_pago === 'Crédito' ? `FACT. CRED (${dataFactura.dias_credito || 0}D)` : "FACTURA CONTADO";
            doc.text(tipoText, 113, y, alignCenter); y += 12;

            doc.setFontSize(11);
            doc.text(`N° ${dataFactura.serie}-${dataFactura.numero}`, 113, y, alignCenter); y += 12;

            doc.setFontSize(8).setFont('helvetica', 'normal');
            doc.text(`Fecha: ${new Date(dataFactura.fecha).toLocaleString()}`, 113, y, alignCenter); y += 10;
        ...
        doc.setFont('helvetica', 'normal');
        dataFactura.detalles.forEach(item => {
            const art = item.articulo || {};
            // Cantidad + Nombre articulo (truncate if too long)
            ...
            // Si hay segunda linea de descripcion
        ...
        doc.text(formatCurrency(dataFactura.total_ventas), mLeft + width, y, { align: 'right' }); y += 12;
        
        const iPorcentaje = dataFactura.impuesto_obj?.porcentaje || 0;
        doc.text(`IVA (${iPorcentaje}%):`, mLeft + 60, y); 
        doc.text(formatCurrency(dataFactura.impuesto_total), mLeft + width, y, { align: 'right' }); y += 12;
        
        doc.setFontSize(11);
        doc.text("TOTAL C$:", mLeft + 50, y); 
        doc.text(formatCurrency(dataFactura.total_factura), mLeft + width, y, { align: 'right' }); y += 15;

        // --- Conversión a Dólar ---
        const totalUSD = tipoCambio > 0 ? (dataFactura.total_factura / tipoCambio) : 0;
        ...
        const txtGlobal = empresa?.mensaje_factura || 'GRACIAS POR SU COMPRA.';
    ...
    const facturasFiltradasLista = facturas.filter(f =>
        f.numero.includes(searchTerm) ||
        (f.cliente && f.cliente.nombre.toLowerCase().includes(searchTerm.toLowerCase()))
    );
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 pb-12 w-full max-w-7xl mx-auto">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-slate-200 dark:border-slate-800 pb-4 gap-4">
                        <div className="flex items-center space-x-3">
                            ...
                            <div>
                                ...
                                <p className="text-slate-500 dark:text-slate-400">Emisión de facturas y descargo de inventario.</p>
                            </div>
                        </div>
                        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                            <div className="relative">
                                ...
                                <input
                                    type="text" placeholder="Buscar factura o cliente..."
                                    className="pl-10 pr-4 py-2 w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-sm outline-none focus:ring-2 focus:ring-emerald-500"
                            ...
                            <button
                                ...
                                className="flex items-center justify-center px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-bold shadow-lg shadow-emerald-500/20 transition-all text-sm w-full sm:w-auto"
                            >
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-left whitespace-nowrap">
                                ...
                                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                    {facturasFiltradasLista.length === 0 ? (
                                        <tr><td colSpan="6" className="px-6 py-8 text-center text-slate-500">No hay facturas registradas en el sistema.</td></tr>
                                    ) : (
                                        facturasFiltradasLista.map(f => (
                                            <tr key={f._id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                                                <td className="px-6 py-4">
                                                    <span className="font-bold text-slate-800 dark:text-slate-200">{f.serie}-{f.numero}</span>
                                                </td>
                                                <td className="px-6 py-4 text-slate-600 dark:text-slate-400">
                                                    {new Date(f.fecha).toLocaleDateString()}
                                                </td>
                                                <td className="px-6 py-4 text-slate-800 dark:text-slate-200 font-medium truncate max-w-[200px]">
                                                    {f.cliente ? f.cliente.nombre : 'Consumidor Final'}
                                                </td>
                                                <td className="px-6 py-4 text-center">
                                                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold ${f.estado === 'Emitida' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400' :
                                                        f.estado === 'Anulada' ? 'bg-rose-100 text-rose-700 dark:bg-rose-500/20 dark:text-rose-400' :
                                                ...
                                                <td className="px-6 py-4 text-right font-mono font-bold text-slate-900 dark:text-white">
                                                    {formatMoney(f.total_factura)}
                                                </td>
                                                <td className="px-6 py-4 text-center">
                                                    <div className="flex justify-center flex-nowrap space-x-2">
                                                            <button
                                                                onClick={() => setModalReimpresion({ doc: f, tipo: 'Factura' })}
                                                                className="p-1.5 text-slate-500 hover:text-emerald-600 bg-slate-100 dark:bg-slate-800 hover:bg-emerald-100 dark:hover:bg-emerald-900 rounded"
                                                        ...
                                                        {f.estado === 'Emitida' && (
                                                            <button
                                                                onClick={() => handleAnularFactura(f._id)}
                                                                className="p-1.5 text-slate-500 hover:text-rose-600 bg-slate-100 dark:bg-slate-800 hover:bg-rose-100 dark:hover:bg-rose-900 rounded"
                                                                title="Anular (Devuelve Stock)"
                                                            >
                                                                <XCircle size={16} />
                                                            </button>
                                                        )}
            ...
            {/* ====== VISTA: FORMULARIO DE NUEVA FACTURA ====== */}
            {viewMode === 'form' && (
                <div className="animate-in fade-in duration-300 space-y-6">
                    <div className="flex justify-between items-center border-b border-slate-200 dark:border-slate-800 pb-4">
                        <div className="flex items-center space-x-3">
                            <button
                                ...
                                className="p-2 border border-slate-200 dark:border-slate-700 text-slate-500 hover:text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-500/20 rounded-xl transition-all"
                            >
                                <ArrowLeft size={20} />
                            </button>
                            <div>
                                <h1 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center">
                                    Facturando Oficialmente
                                    {formData.proformaIdOrigen && (
                                        <span className="ml-3 px-2 py-0.5 bg-indigo-100 text-indigo-700 text-xs rounded-full font-bold">Desde Proforma</span>
                                    )}
                                </h1>
                                <p className="text-xs text-slate-500 dark:text-slate-400">Esta acción afectará al stock de bodega inmediatamente.</p>
                            </div>
                        </div>
                        ...
                        <div className="flex space-x-3">
                            {puedeElegirFormato && (
                                <div className="flex flex-col">
                                    ...
                                    <select 
                                        className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-xs font-bold rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-emerald-500 shadow-sm transition-all"
                                    ...
                                    >
                                        {formatosImpresion.length > 0 ? (
                                            ...
                                            <option value="">Sin formatos de Factura</option>
                                        )}
                                    </select>
                                </div>
                            )}
                            <button
                                ...
                                className="flex items-center px-4 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 dark:bg-indigo-500/10 dark:hover:bg-indigo-500/20 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-800 rounded-xl font-bold transition-colors text-sm"
                            >
                                <FileInput size={16} className="mr-2" /> Importar Proforma
                            </button>
                            <button
                                onClick={handleGuardarImprimir}
                                className="flex items-center px-6 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-bold shadow-lg shadow-emerald-500/20 active:scale-95 transition-all text-sm"
                            >
                    ...
                    <div className="flex flex-col xl:flex-row gap-6">
                        <div className="w-full lg:w-1/3 flex flex-col h-auto lg:h-[calc(100vh-140px)] space-y-4">
                            <div className="bg-white dark:bg-slate-900/60 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
                                <h2 className="font-bold text-slate-800 dark:text-slate-200 flex items-center mb-4 text-sm">
                                    <Box size={16} className="mr-2 text-emerald-500" /> Retención de Configuración
                                </h2>

                                <div>
                                    <label className="block text-xs font-bold text-slate-500 mb-1">Caja de Salida (*)</label>
                                    <select
                                        className="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white focus:ring-2 focus:ring-emerald-500 outline-none"
                            ...
                            <div className="bg-white dark:bg-slate-900/60 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
                                <h2 className="font-bold text-slate-800 dark:text-slate-200 flex items-center mb-4 text-sm">
                                    <User size={16} className="mr-2 text-emerald-500" /> Cliente Compra
                                </h2>

                                <div>
                                    <label className="block text-xs font-bold text-slate-500 mb-1">Asignar Cliente (*)</label>
                                    <select
                                        className="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white outline-none font-bold"
                        ...
                        <div className="w-full xl:w-2/3 flex flex-col gap-6">
                            ...
                            <div className="bg-white dark:bg-slate-900/60 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col h-full min-h-[400px]">
                                <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/30">
                                    <h3 className="font-bold text-slate-800 dark:text-slate-200 text-sm">Detalle de Venta</h3>
                                    <button
                                        onClick={toggleArticulosModal}
                                        className="px-3 py-1.5 bg-emerald-100 hover:bg-emerald-200 dark:bg-emerald-500/20 dark:hover:bg-emerald-500/30 text-emerald-700 dark:text-emerald-400 rounded-lg text-xs font-bold transition-colors flex items-center"
                                    >
                                        <Plus size={14} className="mr-1" /> + Agregar Repuestos
                                    </button>
                                </div>

                                {/* Items Seleccionados (Scrollable Area) */}
                                <div className="flex-1 overflow-y-auto p-0 space-y-3 max-h-[40vh] lg:max-h-none min-h-[150px]">
                                    {carrito.length === 0 ? (
                                        <div className="px-4 py-16 text-center text-slate-400 text-sm">
                                            <Calculator size={32} className="mx-auto mb-3 opacity-20" />
                                            Venta vacía. Agrega artículos o importa una proforma.
                                        </div>
                                    ) : (
                                        <table className="w-full text-left whitespace-nowrap">
                                            <thead className="bg-slate-100 dark:bg-slate-800 sticky top-0 z-10 text-[10px] font-bold text-slate-500 uppercase tracking-wider border-b border-slate-200 dark:border-slate-700">
                                                <tr>
                                                    ...
                                                    <th className="px-4 py-3 min-w-[150px]">Articulo Desc.</th>
                                                    ...
                                                    <th className="px-4 py-3 text-right w-28">SubT.</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                {carrito.map((item, idx) => {
                                                    const esStockAlerta = item.cantidad > item.articulo.stock;
                                                    return (
                                                        <tr key={idx} className={`border-b border-slate-100 dark:border-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-800/30 ${esStockAlerta ? 'bg-rose-50 dark:bg-rose-900/10' : ''}`}>
                                                            ...
                                                            <td className="px-4 py-2">
                                                                <p className="text-sm font-medium text-slate-800 dark:text-slate-200 truncate max-w-[180px]" title={item.articulo.nombre}>{item.articulo.nombre}</p>
                                                            ...
                                                            <td className="px-4 py-3 text-center">
                                                                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${esStockAlerta ? 'bg-rose-200 text-rose-800' : 'bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300'}`}>
                                                                    D: {item.articulo.stock}
                                                                </span>
                                                            </td>
                                                            <td className="px-2 py-2 text-center">
                                                                <input
                                                                    ...
                                                                    className={`w-12 bg-white dark:bg-slate-900 border ${esStockAlerta ? 'border-rose-500 animate-pulse' : 'border-slate-300 dark:border-slate-700'} rounded-md px-1 py-1 text-center text-sm font-bold outline-none`}
                                                                />
                                                            </td>
                                                            <td className="px-4 py-2 text-right">
                                                                <input
                                                                    ...
                                                                    className={`w-20 bg-transparent border-b border-transparent ${!esAdmin ? 'text-slate-400 cursor-not-allowed' : 'hover:border-slate-300 dark:hover:border-slate-600 focus:border-emerald-500 font-bold border-emerald-500/30'} px-1 py-1 text-right text-sm font-mono outline-none`}
                                                                />
                                                            </td>
                                                            <td className="px-4 py-2 text-right">
                                                                <input
                                                                    ...
                                                                    className="w-12 bg-transparent border-b border-transparent hover:border-slate-300 dark:hover:border-slate-600 focus:border-emerald-500 px-1 py-1 text-right text-sm font-mono text-rose-500 outline-none font-bold"
                                                                />
                                                            </td>
                                ...
                                {/* Panel Totales (Fijo Abajo en Desktop, Normal en Móvil pero visible) */}
                                <div className="bg-slate-50 dark:bg-slate-800/50 p-4 border-t border-slate-200 dark:border-slate-800 rounded-b-2xl shadow-[0_-10px_15px_-3px_rgba(0,0,0,0.05)] dark:shadow-[0_-10px_15px_-3px_rgba(0,0,0,0.2)] mt-auto z-10 sticky bottom-0">

                                    <div className="space-y-2 w-full md:w-1/2 lg:w-1/3 ml-auto">
                                        ...
                                        <div className="flex justify-between items-center text-xl text-slate-900 dark:text-white border-t border-slate-200 dark:border-slate-700 pt-2">
            ...
            {/* ====== MODAL IMPORTAR PROFORMA ====== */}
            {isImportModalOpen && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white dark:bg-slate-900 w-full max-w-3xl rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col max-h-[85vh]">
                        <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
                            <h2 className="text-xl font-bold text-slate-900 dark:text-white flex items-center">
                                <FileText className="mr-2 text-indigo-500" size={20} />
                                Importar desde Proforma
                            </h2>
                            <button onClick={toggleImportModal} className="p-1 text-slate-400 hover:text-slate-700 dark:hover:text-white">
                                <XCircle size={24} />
                            </button>
                        </div>
                        ...
                        <div className="p-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
                            <div className="relative">
                                <Search className="absolute left-3 top-2.5 text-slate-400" size={18} />
                                <input
                                    type="text" placeholder="Buscar Cliente o Número de Proforma emitida..."
                                    className="w-full pl-10 pr-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 focus:border-indigo-500 rounded-xl outline-none shadow-sm"
                                    value={proformaSearchTerm} onChange={e => setProformaSearchTerm(e.target.value)}
                                />
                            </div>
                        </div>
                        ...
                        <div className="overflow-y-auto p-0 flex-grow">
                            <table className="w-full text-left whitespace-nowrap">
                                ...
                                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                    {proformasFiltradasImport.length === 0 ? (
                                        <tr><td colSpan="5" className="px-6 py-8 text-center text-slate-500">No hay proformas emitidas disponibles.</td></tr>
                                    ) : (
                                        proformasFiltradasImport.map(p => (
                                            <tr key={p._id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30">
                                                <td className="px-6 py-3 font-bold text-slate-800 dark:text-slate-200">PRF-{p.numero}</td>
                                                <td className="px-6 py-3 truncate max-w-[200px] text-sm text-slate-600 dark:text-slate-300">{p.cliente ? p.cliente.nombre : 'N/A'}</td>
                                                <td className="px-6 py-3 text-xs text-slate-500">{new Date(p.fecha).toLocaleDateString()}</td>
                                                <td className="px-6 py-3 text-right font-mono font-bold text-indigo-600 dark:text-indigo-400">{formatMoney(p.total)}</td>
                                                <td className="px-6 py-3 text-center">
                                                    <button
                                                        onClick={() => importarProformaAlCarrito(p)}
                ...
                <div className="fixed inset-0 z-50 flex items-start justify-center p-4 sm:p-6 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200 overflow-y-auto">
                    <div className="bg-white dark:bg-slate-900 w-full max-w-4xl rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col my-auto h-[90vh]">
                        ...
                        <div className="p-4 bg-slate-50 dark:bg-slate-800/30 border-b border-slate-200 dark:border-slate-800">
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full pl-12 pr-4 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-emerald-500 rounded-xl text-slate-900 dark:text-white outline-none shadow-sm font-medium transition-all"
                            ...
                            <div className="space-y-2">
                                {articulosFiltrados.slice(0, 50).map(a => {
                                    ...
                                    return (
                                        <div 
                                            key={a._id} 
                                            className={`bg-white dark:bg-slate-900/40 border border-slate-100 dark:border-slate-800 rounded-xl p-3 hover:border-emerald-500/50 hover:bg-emerald-50/10 dark:hover:bg-emerald-500/5 transition-all flex items-center gap-4 group cursor-pointer ${sinStock ? 'opacity-60' : ''}`}
                                            onClick={() => {
                                                if (!sinStock) {
                                                    agregarAlCarrito(a);
                                                } else {
                                                    showToast("Sin existencias en inventario", "error");
                                                }
                                            }}
                                        >
                                            ...
                                            <div className="flex-grow min-w-0">
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    <span className="text-[10px] bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 px-2 py-0.5 rounded font-bold font-mono">
                                                        CÓD: {a.codigo}
                                                    </span>
                                                    {a.numeroParte && (
                                            ...
                                            <button className={`p-2 bg-slate-100 dark:bg-slate-800 rounded-xl transition-all shadow-sm ${sinStock ? 'text-slate-300' : 'text-slate-400 group-hover:bg-emerald-600 group-hover:text-white'}`}>
                                                <Plus size={20} />
                                            </button>
                                        </div>
                                    );
                        ...
                        <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 flex justify-end">
                            <button 
                                onClick={toggleArticulosModal}
                                className="px-6 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-bold transition-all shadow-lg shadow-emerald-500/20"
                            >
                                Listo
                            </button>
                        </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl p-6 w-full max-w-sm animate-in zoom-in-95 duration-200">
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400 mb-5">
                            Elige el formato con el que deseas reimprimir este documento.
                        </p>
                        <select
                            className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 text-sm font-semibold rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-emerald-500 mb-5"
                            value={formatoSeleccionado}
                            onChange={(e) => setFormatoSeleccionado(e.target.value)}
                        >
                            {formatosImpresion.length > 0 ? (
                        ...
                        <div className="flex gap-3">
                            ...
                            <button
                                ...
                                className="flex-1 px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-bold shadow-lg shadow-emerald-500/20 transition-all"
                            >
                                🖨️ Imprimir
                            </button>
                        </div>
